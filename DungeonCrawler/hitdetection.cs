...
namespace ZombieGame{
    public class HitDetection{
        ...
        public void FieldDetection(LevelCreator level, Player player,
                    ZombieSpawner zombiespawner, Shooting shooting, KeyboardInput keypress){
            ...
            for (int i = 0; i < fields.Count; i++){
                for (int j = 0; j < fields[i].Count; j++) {
                    ...
                    if (field.getwall){
                        ...
                        //     }
                        // }
                    }else if (field.treasure && coordinate.DistanceTo(player.getX, field.getX, player.getY, field.getY)< 15){
